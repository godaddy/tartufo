        ...
        if not self.signature_is_excluded(string, chunk.file_path):
            ...
            if entropy_score > min_entropy_score:
                if self.entropy_string_is_excluded(string, line, chunk.file_path):
                    ...
                    and self.entropy_string_is_excluded(
                        backwards_compatibility_prefix + string, line, chunk.file_path
                    )
                ):
                    self.logger.debug(
                    ...
                    new_issue = Issue(types.IssueType.Entropy, string, chunk)
                    compat_issue = Issue(
                        types.IssueType.Entropy,
                        backwards_compatibility_prefix + string,
                        chunk,
                    )
                    warnings.warn(
                        f"Signature {compat_issue.signature} was generated by an "
                        "old version of tartufo and will be deprecated in a future "
                        "tartufo version. To avoid problems in the future, update "
                        f"your configuration to use signature {new_issue.signature} "
                        "instead.",
                        DeprecationWarning,
                    )
